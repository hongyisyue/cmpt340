-- File Name: studentDatabaseFull.hs
-- Author: David Kreiser
-- Class: CMPT 340 Tutorial 5
-- Solution to exercises on slide 11
...
-- In this case, students are only "equal" if their IDNum is the same
studentEq :: Student -> Student -> Bool
studentEq (Active _ _ id1 _) (Active _ _ id2 _)   = id1 == id2
studentEq (Withdrawn _ _ id1) (Withdrawn _ _ id2) = id1 == id2
...
courseAverage (Course _ studentList) = calculateCourseAverage studentList 0 0
    where calculateCourseAverage :: [Student] -> Double -> Double -> Double
          calculateCourseAverage [] averages students     = averages / students
          calculateCourseAverage (x:xs) averages students | isActive x = calculateCourseAverage xs (averages + getAverage x) (students + 1)
                                                          | otherwise  = calculateCourseAverage xs averages students
